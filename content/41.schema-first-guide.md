...
最终我们会得到一个与你可能已经通过 [“code first” 指南](/guide/introduction)构建过的同款应用，但会再次从零开始重建。提前阅读该指南并不是严格必需的，不过我们会在多个地方引用它作为对比。
We'll end with the same application that you may have already created by following [the "code first" guide](/guide/introduction), but re-create it from scratch again. Reading that guide beforehand is not strictly required, but we will make several references back to it as a point of comparison.
...
我们会采用与 [“code first” 指南](/guide/introduction)类似的项目搭建方式。
We will use a similar setup to [the "code first" guide](/guide/introduction).
...
就像在 [“code first” 指南](/guide/first-entity#ecmascript-modules)中一样，我们会使用 ECMAScript Modules。请确保在 `package.json` 中包含：

Just [as in the "code first" guide](/guide/first-entity#ecmascript-modules), we'll be using ECMAScript Modules. Make sure you have
...
我们会使用几乎与 [“code first” 指南](/guide/first-entity#configuring-typescript)相同的配置。如同那里提到的，如果你清楚自己在做什么，可以按需调整这份配置。

We will use almost the same config [as the "code first" guide one](/guide/first-entity#configuring-typescript). As mentioned there, already, adjust this config if you know what you’re doing.
