...
在当前版本中，MikroORM 可以自动对 [references](/guide/type-safety #reference-wrapper) 与 [collections](./collections.md) 进行批量处理。不过，如果你有兴趣尝试更进一步的能力，还有一个独立于主仓库的库 [mikro-orm-dataloaders](https://github.com/darkbasic/mikro-orm-dataloaders)，它可以对完整的 find() queries 做批量处理，并支持其中一部分 operators。
In the current version, MikroORM is able to automatically batch [references](/guide/type-safety #reference-wrapper) and [collections](./collections.md). However, if you are interested to try it out there is an [out of tree library](https://github.com/darkbasic/mikro-orm-dataloaders) which takes care of batching whole find queries with a subset of the operators supported.
...
`DataloaderType.REFERENCE` 会为 [References](/guide/type-safety #reference-wrapper) 启用数据加载器，`DataloaderType.COLLECTION` 会为 [Collections](./collections.md) 启用数据加载器，而 `DataloaderType.ALL` 会同时为两者启用。你也可以传入 boolean 值来启用/禁用全部数据加载器。
`DataloaderType.REFERENCE` enables the dataloader for [References](/guide/type-safety #reference-wrapper), `DataloaderType.COLLECTION` enables it for [Collections](./collections.md) while `DataloaderType.ALL` enables it for both. A boolean value is also supported to enable/disable all of them.
...
ManyToOne 与 OneToOne relations 需要使用 [Reference wrapper](/guide/type-safety #reference-wrapper)：
ManyToOne and OneToOne relations require the use of the [Reference wrapper](/guide/type-safety #reference-wrapper):
