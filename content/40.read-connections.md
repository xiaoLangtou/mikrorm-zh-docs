...
你可以通过 `replicas` 选项指定多个读连接。你只需要提供与主连接（master connection）不同的字段，其余字段会从主连接继承。
...
在解析读连接时，默认策略是：对所有不在事务中的读操作（SELECT、COUNT）随机分配一个读副本。
...
你可以在对应 Options 参数（例如 `FindOptions`、`CountOptions`）上使用 `connectionType` 属性，为 find 与 count 操作显式指定连接类型。
...
你也可以将连接解析策略反转：把 `preferReadReplicas` 配置项设为 `false`，这样默认连接将始终是写连接，除非你显式请求读连接（这在“有读副本但只希望特定用例才使用”的应用里很有用）。
...
默认情况下，如果不在事务中，select 查询会使用随机的读连接。你也可以在 `em.getConnection(type: 'read' | 'write')` 中显式指定连接类型。
