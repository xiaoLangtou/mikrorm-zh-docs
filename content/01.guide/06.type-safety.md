...
`isInitialized()` 方法可用于运行时检查，但这可能最终会相当繁琐——我们可以做得更好！不需要手动检查 Entity 状态，我们可以使用 [`Reference`](/api/core/class/Reference) 包装器。<br/>
> _The `isInitialized()` method can be used for runtime checks, but that could end up being quite tedious - we can do better! Instead of manual checks for entity state, we can use the [`Reference`](/api/core/class/Reference) wrapper._
...
你可以通过使用 [`Reference`](/api/core/class/Reference) 包装器来克服这个问题。它简单地封装了 Entity，定义了 `load(): Promise<T>` 方法，如果关联还不可用，将首先延迟加载它。你还可以使用 `unwrap(): T` 方法在不加载的情况下访问底层 Entity。
> _You can overcome this issue by using the [`Reference`](/api/core/class/Reference) wrapper. It simply wraps the entity, defining `load(): Promise<T>` method that will first lazy load the association if not already available. You can also use `unwrap(): T` method to access the underlying entity without loading it._
...
`Loaded` 类型将表示 Entity 的哪些关系已被填充，并将向它们添加一个特殊的 `$` 符号，允许对加载的属性进行类型安全的同步访问。这与 [`Reference`](/api/core/class/Reference) 包装器配合得很好：
...
当你将属性定义为 [`Reference`](/api/core/class/Reference) 包装器时，你需要分配 [`Reference`](/api/core/class/Reference) 实例而不是 Entity。你可以通过 `ref(entity)` 将任何 Entity 转换为 [`Reference`](/api/core/class/Reference) 包装器，或者使用 [`em.getReference()`](/api/core/class/EntityManager#getReference) 的 `wrapped` 选项：
...
从 v5 开始，我们还可以在不访问 [`EntityManager`](/api/core/class/EntityManager) 的情况下创建 Entity 引用。如果你想从 Entity 构造函数内部创建引用，这可能很方便：
...
如果引用已经存在，你需要用新的 [`Reference`](/api/core/class/Reference) 实例重新分配它——它们像 Entity 一样保持标识，因此你需要替换它们：
...
`Ref` 是一个交叉类型，它将主键属性添加到 [`Reference`](/api/core/class/Reference) 接口。它允许直接从 [`Reference`](/api/core/class/Reference) 实例获取主键。
