...
让我们在 `src` 目录中创建新文件 `app.ts`，并从中导出一个 `bootstrap` 函数，在其中创建 fastify app 实例。还记得我们如何通过 fork [`EntityManager`](/api/core/class/EntityManager) 来绕过全局上下文验证吗？对于 Web 服务器，我们可以利用中间件，或者在 fastify 中使用 hook，来自动实现唯一的请求上下文。MikroORM 提供了一个名为 `RequestContext` 的便捷 helper，可用于为每个请求创建 fork。[`EntityManager`](/api/core/class/EntityManager) 知道这个类，并尝试自动从中获取正确的上下文。
...
在内部，所有使用 Identity Map 的 [`EntityManager`](/api/core/class/EntityManager) 方法（例如 [`em.find()`](/api/core/class/EntityManager#find) 或 [`em.getReference()`](/api/core/class/EntityManager#getReference)）首先调用 `em.getContext()` 来访问上下文 fork。此方法将首先检查我们是否在 `RequestContext` 处理程序内运行，并优先使用来自它的 [`EntityManager`](/api/core/class/EntityManager) fork。
...
Node.js 核心的 [`AsyncLocalStorage`](https://nodejs.org/api/async_context.html#class-asynclocalstorage) 类是这里的魔术师。它允许我们在整个异步调用过程中跟踪上下文。它允许我们将 [`EntityManager`](/api/core/class/EntityManager) fork 的创建（通常在中间件中，如上一节所示）与通过全局 [`EntityManager`](/api/core/class/EntityManager) 实例使用它解耦。

_The [`AsyncLocalStorage`](https://nodejs.org/api/async_context.html#class-asynclocalstorage) class from Node.js core is the magician here. It allows us to track the context throughout the async calls. It allows us to decouple the [`EntityManager`](/api/core/class/EntityManager) fork creation (usually in a middleware as shown in the previous section) from its usage through the global [`EntityManager`](/api/core/class/EntityManager) instance._
...
虽然 [`EntityManager`](/api/core/class/EntityManager) 和 [`EntityRepository`](/api/core/class/EntityRepository) 类由 `@mikro-orm/core` 包提供，但那些只是基础——driver 无关的——实现。这意味着什么的一个例子是 `QueryBuilder`——作为 SQL 概念，它在 `@mikro-orm/core` 包中没有位置，相反，[`EntityManager`](/api/core/class/EntityManager) 的扩展称为 `SqlEntityManager` 由 SQL driver 包提供（它在 `@mikro-orm/knex` 包中定义，并在依赖于它的每个 SQL driver 包中重新导出）。这个 `SqlEntityManager` 类提供了额外的 SQL 相关方法，例如 `em.createQueryBuilder()`。
...
_For convenience, the `SqlEntityManager` class is also reexported under [`EntityManager`](/api/core/class/EntityManager) alias. This means we can do `import { EntityManager } from '@mikro-orm/sqlite'` to access it._

_Under the hood, MikroORM will always use this driver-specific [`EntityManager`](/api/core/class/EntityManager) implementation (you can verify that by `console.log(orm.em)`, it will be an instance of `SqlEntityManager`), but for TypeScript to understand it, you will need to use the driver package to import it. The same applies to the [`EntityRepository`](/api/core/class/EntityRepository) and `SqlEntityRepository` classes._
...
Entity repository 是 [`EntityManager`](/api/core/class/EntityManager) 之上的薄层。它们作为扩展点，因此你可以添加自定义方法，甚至更改现有方法。默认的 [`EntityRepository`](/api/core/class/EntityRepository) 实现只是将调用转发给底层的 [`EntityManager`](/api/core/class/EntityManager) 实例。
...
[`EntityRepository`](/api/core/class/EntityRepository) 类携带 Entity 类型，所以我们不必在每次 `find` 或 `findOne` 调用中传递它。
...
请注意，没有“刷新 repository”这种事情——它只是 [`em.flush()`](/api/core/class/EntityManager#flush) 的快捷方式。换句话说，我们总是刷新整个 Unit of Work，而不仅仅是这个 repository 表示的单个 Entity。
...
我们可以使用我们之前描述的 [`em.create()`](/api/core/class/EntityManager#create) 函数。它在返回创建的 Entity 之前实际上调用了 `em.persist(entity)`，所以你甚至不需要对 Entity 本身做任何事情，单独调用 [`em.create()`](/api/core/class/EntityManager#create) 就足够了。是时候测试它了！
