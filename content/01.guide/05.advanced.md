...
代码变得有点混乱了，让我们使用 [`em.create()`](/api/core/class/EntityManager#create) 来使其再次简洁：
...
然后我们定义创建评论的端点——这里我们使用 `getUserFromToken` helper 基于 token 访问当前用户，尝试查找文章（同样基于 `slug` 属性）并创建评论 Entity。由于我们在这里使用 [`em.create()`](/api/core/class/EntityManager#create)，我们不必调用 [`em.persist()`](/api/core/class/EntityManager#persist) 新 Entity，因为这样做会自动完成。
...
虽然我们在本指南中没有这样的用例，但使用 [`EntityManager`](/api/core/class/EntityManager) 和 Unit of Work 方法的一个巨大好处是自动批处理——所有的 `INSERT`、`UPDATE` 和 `DELETE` 查询都将自动批处理为每个 Entity 的单个查询。
...
MikroORM 有一个简单的 [result caching](../caching.md) 机制，你需要做的就是在你的 [`em.find()`](/api/core/class/EntityManager#find) 选项中添加 `cache` 选项。该值可以是以下之一：
