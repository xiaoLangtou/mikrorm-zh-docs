---
title: '第一章：第一个 Entity'
---

## 环境搭建

在我们开始之前，请确保你首先满足以下先决条件：

_Before we start, ensure you meet the following pre-requisites first:_

1. 已安装 Node.js 18.12 或更高版本，但最好是版本 20。

   _Have Node.js version 18.12 or higher installed, but preferably version 20._

   - 访问 [Node.js 官网](https://nodejs.org/en/download) 下载或使用 [fnm](https://github.com/Schniz/fnm)。

     _Visit [Node.js website](https://nodejs.org/en/download) to download or use [fnm](https://github.com/Schniz/fnm)._

2. 已安装 NPM，或使用你选择的任何其他包管理器。

   _Have NPM installed, or use any other package manager of your choice._

   - NPM 与 Node.js 捆绑在一起，所以你应该已经有了。如果没有，请重新安装 Node.js。要使用其他包管理器，请考虑使用 [corepack](https://nodejs.org/api/corepack.html)。

     _NPM comes bundled with Node.js, so you should already have it. If not, reinstall Node.js. To use other package managers, consider using [corepack](https://nodejs.org/api/corepack.html)._

如果不确定，请运行以下命令确认先决条件：

_If not certain, confirm the prerequisites by running:_

```bash
node -v
npm -v
```

## 创建新项目

让我们从基本的文件夹结构开始。正如我们所说，我们将有 3 个模块，每个模块都有自己的目录：

_Let's start with the basic folder structure. As we said we will have 3 modules, each having its own directory:_

```bash
# 创建项目文件夹并 `cd` 进入
# create the project folder and `cd` into it
mkdir blog-api && cd blog-api
# 在 `src` 文件夹内创建模块文件夹
# create module folders, inside `src` folder
mkdir -p src/modules/{user,article,common}
```

现在添加依赖：

_Now add the dependencies:_

```bash npm2yarn
npm install @mikro-orm/core \
            @mikro-orm/sqlite \
            @mikro-orm/reflection \
            fastify
```

以及一些开发依赖：

_And some development dependencies:_

```bash npm2yarn
npm install --save-dev @mikro-orm/cli \
                       typescript \
                       ts-node \
                       @types/node \
                       vitest
```

## ECMAScript Modules（ESM）

你可能听说过 ECMAScript Modules (ESM)，但这可能是你第一次尝试使用它们。请记住——整个生态系统还远未准备好，本指南使用 ESM 主要是为了展示如何实现。许多依赖还没有准备好支持 ESM，通常需要奇怪的解决方案。MikroORM 也不例外——也有一些怪癖，特别是在测试设置下动态导入 TypeScript 文件时。

_You probably heard about ECMAScript Modules (ESM), but this might easily be the first time you try them. Now keep in mind - the whole ecosystem is far away from ready, and this guide is using the ESM mainly to show how it is possible. Many dependencies are not ESM ready, and often there are weird workarounds needed. MikroORM is no exception to this - there are quirks as well, namely in dynamic imports of TypeScript files under test setup._

> 你不必使用 ESM 来使用 MikroORM。MikroORM 可以在 ESM 项目中工作，也可以在 CommonJS (CJS) 项目中工作。
>
> _You do not have to use ESM to use MikroORM. MikroORM can work in ESM projects, as well as CommonJS (CJS) projects._

简而言之，对于 ESM 项目，我们需要：

_In a nutshell, for ESM project we need to:_

- 在 package.json 中添加 `"type": "module"`

  _Add `"type": "module"` to package.json_

- 使用 `import/export` 语句代替 `require` 调用

  _Use `import/export` statements instead of `require` calls_

- 在这些 `import` 中使用 `.js` 扩展名，即使在 TypeScript 文件中也是如此

  _Use `.js` extension in those `import`s, even in TypeScript files_

- 按照下一节中的说明配置 TypeScript 和 `ts-node` 属性

  _Configure TypeScript and `ts-node` property, as described in the following section_

> 你可以在[这里](https://nodejs.org/api/esm.html)阅读更多关于 Node.js 中 ESM 支持的内容。
>
> _You can read more about the ESM support in Node.js [here](https://nodejs.org/api/esm.html)._

除此之外，使用 Decorator 定义 Entity 时有一个需要注意的地方。MikroORM 获取属性类型的默认方式是通过 `reflect-metadata`。虽然这本身就引入了[一些挑战和限制](../metadata-providers#limitations-and-requirements)，但我们无法在 ESM 项目中使用它。这是因为，对于 ES 模块，依赖是异步并行解析的，这与 `reflect-metadata` 模块当前的工作方式不兼容。因此，我们需要使用其他方式来定义 Entity 元数据——在本指南中，我们将使用 `@mikro-orm/reflection` 包，它在底层使用 `ts-morph` 从 TypeScript Compiler API 获取信息。这在 ESM 项目中工作正常，而且还开辟了编译 TypeScript 文件的新方式，比如 `esbuild`（它不支持 Decorator Metadata）。

In addition to this, there is one gotcha with defining entities using decorators. The default way MikroORM uses to obtain a property type is via `reflect-metadata`. While this itself introduces [some challenges and limitations](../metadata-providers#limitations-and-requirements), we can't use it in an ESM project. This is because, with ES modules, the dependencies are resolved asynchronously, in parallel, which is incompatible with how the `reflect-metadata` module currently works. For this reason, we need to use other ways to define the entity metadata - in this guide, we will use the `@mikro-orm/reflection` package, which uses `ts-morph` under the hood to gain information from TypeScript Compiler API. This works fine with ESM projects, and also opens up new ways of compiling the TypeScript files, like `esbuild` (which does not support decorator metadata).

定义 Entity 的另一种方式是通过 [`EntitySchema`](../entity-schema)，这种方法也适用于原生 JavaScript 项目，并且允许通过接口而不是类来定义 Entity。查看[定义 Entity 章节](../defining-entities)，那里的所有示例都有通过 `EntitySchema` 定义的代码标签。

Another way to define your entities is via [`EntitySchema`](../entity-schema), this approach works also for vanilla JavaScript projects, as well as allows to define entities via interfaces instead of classes. Check the [Defining Entities section](../defining-entities), all examples there have code tabs with definitions via `EntitySchema` too.

使用 `ts-morph` 的反射性能开销较大，因此[元数据会被缓存](../metadata-cache.md)到 `temp` 文件夹中，并在你更改 Entity 定义（或更新 ORM 版本）时自动失效。你应该将此文件夹添加到 `.gitignore` 文件中。请注意，当你构建生产包时，你可以利用 CLI 在构建时生成生产缓存，以获得更快的启动时间。有关此内容的更多信息，请参阅[部署章节](../deployment.md)。

The reflection with `ts-morph` is performance heavy, so the [metadata are cached](../metadata-cache.md) into `temp` folder and invalidated automatically when you change your entity definition (or update the ORM version). You should add this folder to `.gitignore` file. Note that when you build your production bundle, you can leverage the CLI to generate production cache on build time to get faster start-up times. See the [deployment section](../deployment.md) for more about this.

## 配置 TypeScript

我们将使用以下 TypeScript 配置，因此创建 `tsconfig.json` 文件并将其复制到那里。如果你知道自己在做什么，你可以调整配置以适应你的需求。

We will use the following TypeScript config, so create the `tsconfig.json` file and copy it there. If you know what you are doing, you can adjust the configuration to fit your needs.

为了使 ESM 支持正常工作，我们需要将 `module` 和 `moduleResolution` 设置为 `NodeNext`，并将目标设置为 `ES2022`。我们还启用了 `strict` 模式和 `experimentalDecorators`，以及 `declaration` 选项来生成 `.d.ts` 文件，这是 `@mikro-orm/reflection` 包所需要的。最后，我们通过 `outDir` 告诉 TypeScript 编译到 `dist` 文件夹，并使其 `include`（包含）`src` 文件夹内的所有 `*.ts` 文件。

For ESM support to work, we need to set `module` and `moduleResolution` to `NodeNext` and target `ES2022`. We also enable `strict` mode and `experimentalDecorators`, as well as the `declaration` option to generate the `.d.ts` files, needed by the `@mikro-orm/reflection` package. Lastly, we tell TypeScript to compile into `dist` folder via `outDir` and make it `include` all `*.ts` files inside `src` folder.

```json title='tsconfig.json'
{
  "compilerOptions": {
    "module": "NodeNext",
    "moduleResolution": "NodeNext",
    "target": "ES2022",
    "strict": true,
    "outDir": "dist",
    "declaration": true,
    "experimentalDecorators": true
  },
  "include": [
    "./src/**/*.ts"
  ]
}
```

### 使用 `ts-node` 直接运行 TypeScript 文件

在开发过程中，你经常想要运行应用程序来测试你的更改。为此，使用 `ts-node` 直接运行 TypeScript 文件很方便，而不是先编译成 JavaScript。默认情况下，`ts-node` 将以 CommonJS 模式运行，因此我们需要通过 `tsconfig.json` 配置它以启用 ESM 支持。为了加快速度，我们可以使用 `transpileOnly` 选项，它禁用类型检查——我们将在编译生产用应用程序时进行类型检查，所以开发时这并不重要（特别是当你有一个 IDE 无论如何都会向你显示错误时）。

During the development, you will often want to run the app to test your changes. For that, it is handy to use `ts-node` to run the TypeScript files directly, instead of compiling the first to JavaScript. By default, `ts-node` will operate in CommonJS mode, so we need to configure it via `tsconfig.json` to enable ESM support. To speed things up, we can use the `transpileOnly` option, which disables type checking - we will get the type checks when compiling the app for production use, so it does not matter that much during development (especially when you have an IDE that shows you the errors anyway).

```json title='tsconfig.json'
{
  "compilerOptions": { ... },
  "include": [ ... ],
  "ts-node": {
    "esm": true,
    "transpileOnly": true
  }
}
```

## 设置 CLI

接下来，我们将设置 MikroORM 的 CLI 配置。此配置之后也会自动导入到你的应用程序中。定义带有明确 `Options` 类型的配置变量，这样你可以获得最佳的类型安全性——自动补全以及对不存在的选项的警告（与使用 `{ ... } as Options` 不同，后者不会对此发出警告）。

Next, we will set up the CLI config MikroORM. This config will be then automatically imported into your app too. Define the config variable with explicit `Options` type, that way you get the best level of type safety - autocomplete as well as warning about not existing options (as opposed to using `{ ... } as Options`, that won't warn you for such).

> 对于测试，你可以导入配置并在评估之前覆盖某些选项。
>
> For tests, you can import the config and override some options before evaluating it.

```ts title='src/mikro-orm.config.ts'
import { Options, SqliteDriver } from '@mikro-orm/sqlite';
import { TsMorphMetadataProvider } from '@mikro-orm/reflection';

const config: Options = {
  // 为简单起见，我们使用 SQLite 数据库，因为它几乎在任何地方都可用
  // for simplicity, we use the SQLite database, as it's available pretty much everywhere
  driver: SqliteDriver,
  dbName: 'sqlite.db',
  // 基于文件夹的发现设置，使用通用文件名后缀
  // folder-based discovery setup, using common filename suffix
  entities: ['dist/**/*.entity.js'],
  entitiesTs: ['src/**/*.entity.ts'],
  // 我们将使用 ts-morph 反射，作为默认 reflect-metadata provider 的替代方案
  // 查看文档了解它们的区别: https://mikro-orm.io/docs/metadata-providers
  // we will use the ts-morph reflection, an alternative to the default reflect-metadata provider
  // check the documentation for their differences: https://mikro-orm.io/docs/metadata-providers
  metadataProvider: TsMorphMetadataProvider,
  // 启用调试模式以记录 SQL 查询和发现信息
  // enable debug mode to log SQL queries and discovery information
  debug: true,
};

export default config;
```

> 注意，我们从 `@mikro-orm/sqlite` 包导入 `Options`——这是 `Options<SqliteDriver>` 的别名。
>
> Note that we are importing `Options` from the `@mikro-orm/sqlite` package - this is an alias to `Options<SqliteDriver>`.

或者，我们可以使用 `defineConfig` 辅助函数，即使在 JavaScript 文件中也应该提供智能提示，无需类型提示：

Alternatively, we can use the `defineConfig` helper that should provide intellisense even in JavaScript files, without the need for type hints:

```ts
import { defineConfig } from '@mikro-orm/sqlite';
import { TsMorphMetadataProvider } from '@mikro-orm/reflection';

// 现在不需要指定 `driver`，它将自动推断
// no need to specify the `driver` now, it will be inferred automatically
export default defineConfig({
   dbName: 'sqlite.db',
   // 基于文件夹的发现设置，使用通用文件名后缀
   // folder-based discovery setup, using common filename suffix
   entities: ['dist/**/*.entity.js'],
   entitiesTs: ['src/**/*.entity.ts'],
   // 我们将使用 ts-morph 反射，作为默认 reflect-metadata provider 的替代方案
   // 查看文档了解它们的区别: https://mikro-orm.io/docs/metadata-providers
   // we will use the ts-morph reflection, an alternative to the default reflect-metadata provider
   // check the documentation for their differences: https://mikro-orm.io/docs/metadata-providers
   metadataProvider: TsMorphMetadataProvider,
   // 启用调试模式以记录 SQL 查询和发现信息
   // enable debug mode to log SQL queries and discovery information
   debug: true,
});
```

将此文件保存到 `src/mikro-orm.config.ts`，这样它将与你应用程序的其余部分一起编译。

Save this file into `src/mikro-orm.config.ts`, so it will get compiled together with the rest of your app.

> 或者，你可以在项目根目录使用 `mikro-orm.config.js` 文件，这样的文件将自动加载。查阅[文档](../quick-start#setting-up-the-commandline-tool)了解更多信息。
>
> Alternatively, you can use `mikro-orm.config.js` file in the root of your project, such a file will get loaded automatically. Consult [the documentation](../quick-start#setting-up-the-commandline-tool) for more info.

```json title='package.json'
{
  "type": "module",
  "dependencies": { ... },
  "devDependencies": { ... }
}
```

最后，添加一些 NPM 脚本以简化开发。我们将通过 `tsc` 构建应用程序，通过 `vitest` 进行测试，并通过 `ts-node` 在本地运行它。ESM 和动态导入有一个需要注意的地方。虽然它对常规 JavaScript 文件工作正常，但一旦我们通过 `ts-node` 或 `vitest/esbuild` 混合 TypeScript 的运行时支持，你就会遇到像 `Unknown file extension ".ts"` 这样的错误。为了解决这个问题，我们可以通过 `NODE_OPTIONS` 环境变量使用 `ts-node/esm` 加载器——但这可能会变得很麻烦，我们可以做得更好——至少对于 CLI，我们有 `mikro-orm-esm` 脚本，它会自动注册 `ts-node/esm` 加载器并禁用实验性警告。

Lastly, add some NPM scripts to ease the development. We will build the app via `tsc`, test it via `vitest` and run it locally via `ts-node`. There is one gotcha with ESM and dynamic imports. While it works fine for regular JavaScript files, once we mix runtime support for TypeScript via `ts-node` or `vitest/esbuild`, you start hitting the wall with errors like `Unknown file extension ".ts"`. To get around that, we can use the `ts-node/esm` loader via `NODE_OPTIONS` environment variable - but that can get ugly, and we can do better - at least for the CLI, we have the `mikro-orm-esm` script, which automatically registers the `ts-node/esm` loader as well as disables the experimental warning.

> 所以请记住——在使用 TypeScript 的 ESM 项目中始终使用 `mikro-orm-esm`。请注意，这需要安装 `ts-node` 依赖，如果你不使用 TypeScript，常规的 `mikro-orm` 脚本就可以正常工作。
>
> So remember - always use `mikro-orm-esm` in the ESM projects with TypeScript. Note that it requires the `ts-node` dependency to be installed, if you don't use TypeScript, the regular `mikro-orm` script will work fine for you.

这个动态导入的问题可能会出现在 CLI 使用和 `vitest` 中。虽然 `--loader` 解决方案适用于 CLI，但我们可以为 `vitest` 使用更原生的 vite 方案，让我们稍后在你开始编写第一个测试时讨论这部分。

This issue with dynamic imports can surface for both the CLI usage and `vitest`. While the `--loader` solution works for the CLI, we can use something more vite-native for `vitest`, let's talk about that part later when you start writing the first test.

> `ts-node` 二进制文件仅适用于较旧的 Node.js 版本，对于 v20 或更高版本，我们需要使用 `node --loader ts-node/esm`。
>
> The `ts-node` binary works only on older Node.js versions, for v20 or above, we need to use `node --loader ts-node/esm` instead.

```json title='package.json'
{
  "type": "module",
  "dependencies": { ... },
  "devDependencies": { ... },
  "scripts": {
    "build": "tsc",
    "start": "node --no-warnings=ExperimentalWarning --loader ts-node/esm src/server.ts",
    "test": "vitest"
  }
}
```

> 我们在 `start` 脚本中引用了 `src/server.ts` 文件，我们稍后会创建它，现在不需要担心。
>
> We refer to a file `src/server.ts` in the `start` script, we will create that later, no need to worry about it right now.

现在通过 `npx mikro-orm-esm debug` 测试 CLI，你应该会看到类似以下内容：

Now test the CLI via `npx mikro-orm-esm debug`, you should see something like the following:

```text
Current MikroORM CLI configuration
 - dependencies:
   - mikro-orm 6.0.0
   - node 20.9.0
   - knex 3.0.1
   - sqlite3 5.1.6
   - typescript 5.3.3
 - package.json found
 - ts-node enabled
 - searched config paths:
   - /blog-api/src/mikro-orm.config.ts (found)
   - /blog-api/dist/mikro-orm.config.js (found)
   - /blog-api/mikro-orm.config.ts (not found)
   - /blog-api/mikro-orm.config.js (not found)
 - configuration found
 - database connection successful
 - will use `entities` array (contains 0 references and 1 paths)
   - /blog-api/dist/**/*.entity.js (not found)
 - could use `entitiesTs` array (contains 0 references and 1 paths)
   - /blog-api/src/**/*.entity.ts (not found)
```

这看起来不错，我们得到了关于安装内容的完整摘要，可以看到配置被正确加载，并且正如预期的那样，没有发现任何 Entity——因为你需要先创建它们！

This looks good, we get a nice summary of what is being installed, we can see the config being loaded correctly, and as expected, no entities were discovered - because you need to create them first!

> 如果你使用 `npx mikro-orm debug` 而不是 `npx mikro-orm-esm debug`，配置将无法加载，并会出现类似以下错误：
> ```text
> Unknown file extension ".ts" for ./blog-api/src/mikro-orm.config.ts
> ```
>
> If you used `npx mikro-orm debug` instead of `npx mikro-orm-esm debug`, the configuration would fail to be loaded and an error similar to this one would be present:
> ```text
> Unknown file extension ".ts" for ./blog-api/src/mikro-orm.config.ts
> ```

然后测试 TypeScript 构建，因为我们现在有了第一个可以编译的文件。使用 `npm run build` 并检查是否生成了包含配置文件 JavaScript 版本的 `dist` 文件夹。

Then test the TypeScript build, as we now have the first file we can compile. Use `npm run build` and check if the `dist` folder gets generated with the JavaScript version of our config file.

在我们开始创建第一个 Entity 之前，让我们做一个快速的完整性检查——这是我们到目前为止的初始目录结构：

Before we get to creating the very first entity, let's do a quick sanity check - this is our initial directory structure so far:

```text
.
├── dist
│   └── mikro-orm.config.js
├── node_modules
├── package-lock.json
├── package.json
├── src
│   ├── mikro-orm.config.ts
│   └── modules
│       ├── article
│       ├── common
│       └── user
└── tsconfig.json
```

## 第一个 Entity

这是相当多的设置工作，但不要担心，大部分繁重的工作已经完成了。是时候创建你的第一个 Entity 了——`User`！在 `src/modules/user` 中创建一个 `user.entity.ts` 文件，内容如下：

This was quite a lot of setup, but don't worry, most of the heavy lifting is behind you. Time to create your first entity - the `User`! Create a `user.entity.ts` file in `src/modules/user` with the following contents:

:::caution 警告 / Caution
查看[定义 Entity](../defining-entities.md) 章节，它提供了各种属性类型的许多示例以及定义 Entity 的不同方式。

Check out the [Defining Entities](../defining-entities.md) section which provides many examples of various property types as well as different ways to define your entities.
:::

```ts title='user.entity.ts'
import { Entity, PrimaryKey, Property } from '@mikro-orm/core';

@Entity()
export class User {

   @PrimaryKey()
   id!: number;

   @Property()
   fullName!: string;

   @Property()
   email!: string;

   @Property()
   password!: string;

   @Property({ type: 'text' })
   bio = '';

}
```

那么我们这里有什么呢？Entity 是一个 JavaScript 类，使用 `@Entity()` Decorator 装饰，并用其他 Decorator（如 `@Property()`）定义属性。Entity 代表一个数据库表，属性代表其列。

So what do we have here? An entity is a JavaScript class, decorated with an `@Entity()` decorator, that defines properties with other decorators (like `@Property()`). An entity represents a database table, and the properties represent its columns.

> 我们使用 `*.entity.ts` 后缀以便在模块边界之间进行基于文件夹的发现。或者，你可以在 ORM 配置中明确提供 Entity 类引用，例如 `entities: [User]`。通过显式设置，事情会更加流畅且不易出错，没有动态导入，也不涉及文件系统。但基于文件夹的发现可能很方便，特别是当我们的应用程序增长到许多 Entity 时。
>
> We use the `*.entity.ts` suffix for easy folder-based discovery across module boundaries. Alternatively, you could explicitly provide the entity class references in the ORM config, e.g. `entities: [User]`. With the explicit setup, things are more streamlined and less error-prone, there is no dynamic importing, and no file system is involved. But folder-based discovery can be handy, especially when our app grows to many entities.

### 定义主键

每个 Entity 都需要有一个主键，我们将使用一个简单的自增数字主键。当 MikroORM 看到一个 `number` 类型的单一主键属性时，它默认会这样做，所以执行以下操作就足够了：

Every entity needs to have a primary key, we will use a simple auto-increment numeric one. MikroORM defaults to that when it sees a single primary key property with a `number` type, so doing the following is enough:

```ts
@PrimaryKey()
id!: number;
```

如果你想使用 `bigint` 列类型，只需在 Decorator 选项中传递 `type: 'bigint'`。`BigInt` 会被映射为 `string`，因为它们无法安全地放入 JavaScript 的 `number` 中。

In case you want to use `bigint` column type, just pass `type: 'bigint'` in the decorator options. `BigInt`s are mapped to `string`, as they would not fit into JavaScript `number` safely.

```ts
@PrimaryKey({ type: 'bigint' })
id!: string;
```

另一个常见的用例是 UUID。我们可以利用 MikroORM 在创建托管 Entity 实例（从数据库加载的那些）时从不调用 Entity 构造函数这一事实。这意味着属性初始化器（或一般的构造函数）仅对将产生 `INSERT` 查询的 Entity 执行。

Another common use case is UUID. We can leverage the fact, that MikroORM never calls your entity constructor when creating managed entity instances (those loaded from your database). This means property initializers (or in general constructors) are executed only for entities that will produce an `INSERT` query.

```ts
@PrimaryKey({ type: 'uuid' })
uuid = uuid.v4();
```

### 标量属性

要映射常规数据库列，我们可以使用 `@Property()` Decorator。它的工作方式与上面描述的 `@PrimaryKey()` Decorator 相同。你可以说它扩展了它——所有可以传递给 `@Property()` Decorator 的属性也可以在 `@PrimaryKey()` 中使用。

To map regular database columns we can use the `@Property()` decorator. It works the same as the `@PrimaryKey()` decorator describer above. You could say it extends it - all the properties you can pass to the `@Property()` decorator are also available in `@PrimaryKey()` too.

> 我们使用的是 ts-morph Metadata Provider，它有助于高级类型推断。如果你想使用基于 `reflect-metadata` 的默认 Metadata Provider，请查看[文档](../metadata-providers#limitations-and-requirements)了解差异。
>
> We are using the ts-morph metadata provider, which helps with advanced type inference. Check out [the documentation](../metadata-providers#limitations-and-requirements) for the differences if you'd like to use the default metadata provider which is based on `reflect-metadata`.

ORM 将自动将 `string` 属性映射为 `varchar`，对于 `User.bio`，我们希望使用 `text`，因此我们通过 `type` Decorator 选项进行更改：

The ORM will automatically map `string` properties to `varchar`, for the `User.bio` we want to use `text` instead, so we change it via the `type` decorator option:

```ts
@Property({ type: 'text' })
bio = '';
```

这里的 `type` 选项允许多种输入形式。我们在这里使用 `text` 类型名称，它被映射到 `TextType`——ORM 内部使用的映射类型表示。如果你在那里提供一个字符串值，并且它不匹配任何已知的类型别名，它将被视为列类型。我们也可以提供类型类实现而不是 `string` 类型名称：

The `type` option here allows several input forms. We are using the `text` type name here, which is mapped to the `TextType` - a mapped type representation used internally by the ORM. If you provide a string value there, and it won't match any known type alias, it will be considered as the column type. We can also provide a type class implementation instead of a `string` type name:

```ts
import { TextType } from '@mikro-orm/core';

@Property({ type: TextType })
bio = '';
```

还有一个 `types` 映射（为了简洁也导出为 `t`）：

There is also a `types` map (exported also as just `t` for brevity):

```ts
import { t } from '@mikro-orm/core'; // `t` or `types`

@Property({ type: t.text })
bio = '';
```

我们始终可以提供显式的列类型（这甚至可以与 `type` 选项结合使用，以覆盖映射类型的实现方式）：

We can always provide the explicit column type (and this can be even combined with the `type` option, to override how the mapped type is implemented):

```ts
import { t } from '@mikro-orm/core'; // `t` or `types`

@Property({ columnType: 'character varying(1000)' })
bio = '';
```

使用 `columnType` 时，要注意 `length` 或 `precision/scale` 等选项——`columnType` 始终按原样使用，不做任何修改。这意味着你需要在那里传递最终值，包括长度，例如 `columnType: 'decimal(10,2)'`。

When using the `columnType`, be careful about options like `length` or `precision/scale` - `columnType` is always used as-is, without any modification. This means you need to pass the final value there, including the length, e.g. `columnType: 'decimal(10,2)'`.

## 初始化 ORM

最后缺少的步骤是初始化 [`MikroORM`](https://mikro-orm.io/api/core/class/MikroORM) 以访问 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 和其他便捷工具（如 [`SchemaGenerator`](https://mikro-orm.io/api/knex/class/SqlSchemaGenerator)）。

The last missing step is to initialize the [`MikroORM`](https://mikro-orm.io/api/core/class/MikroORM) to get access to the [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) and other handy tools (like the [`SchemaGenerator`](https://mikro-orm.io/api/knex/class/SqlSchemaGenerator)).

```ts title='server.ts'
import { MikroORM } from '@mikro-orm/sqlite'; // 或任何其他驱动包
// or any other driver package

// 初始化 ORM，动态加载配置文件
// initialize the ORM, loading the config file dynamically
const orm = await MikroORM.init();
console.log(orm.em); // 通过 `em` 属性访问 EntityManager
                      // access EntityManager via `em` property
console.log(orm.schema); // 通过 `schema` 属性访问 SchemaGenerator
                         // access SchemaGenerator via `schema` property
```

我们在没有任何参数的情况下使用了 [`init()`](https://mikro-orm.io/api/core/class/MikroORM#init) 方法，这导致 ORM 自动加载 CLI 配置。更明确的方式，它与以下代码相同：

We used the [`init()`](https://mikro-orm.io/api/core/class/MikroORM#init) method without any parameters, which results in the ORM loading the CLI config automatically. In a more explicit way, it's the same as the following code:

```ts title='server.ts'
import { MikroORM } from '@mikro-orm/sqlite';
import config from './mikro-orm.config.js';

const orm = await MikroORM.init(config);
```

:::caution 警告 / Caution：同步初始化 / Synchronous Init

与异步的 [`MikroORM.init`](https://mikro-orm.io/api/core/class/MikroORM#init) 方法相反，你可以选择使用同步变体 [`initSync()`](https://mikro-orm.io/api/core/class/MikroORM#initSync)。此方法有一些限制：

_As opposed to the async [`MikroORM.init`](https://mikro-orm.io/api/core/class/MikroORM#init) method, you can prefer to use synchronous variant [`initSync()`](https://mikro-orm.io/api/core/class/MikroORM#initSync). This method has some limitations:_

- 数据库连接将在你首次与数据库交互时建立（或者你可以显式使用 [`orm.connect()`](https://mikro-orm.io/api/core/class/MikroORM#connect)）

  _Database connection will be established when you first interact with the database (or you can use [`orm.connect()`](https://mikro-orm.io/api/core/class/MikroORM#connect) explicitly)_

- 不加载 `config` 文件，`options` 参数是必需的

  _No loading of the `config` file, `options` parameter is mandatory_

- 不支持基于文件夹的发现

  _No support for folder-based discovery_

- 不检查不匹配的包版本

  _No check for mismatched package versions_

:::

## 使用 Entity Manager

现在你可以访问 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 了，让我们谈谈它是如何工作的以及你如何使用它。

So now you have the access to [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager), let's talk about how it works and how you can use it.

### Persist 和 Flush

我们应该首先描述两个方法来理解 MikroORM 中的持久化是如何工作的：[`em.persist()`](#persist) 和 [`em.flush()`](#flush)。

There are 2 methods we should first describe to understand how persisting works in MikroORM: [`em.persist()`](#persist) and [`em.flush()`](#flush).

[`em.persist(entity)`](#persist) 用于标记新 Entity 以供将来持久化。它将使 Entity 由 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 管理，一旦调用 `flush`，它将被写入数据库。

[`em.persist(entity)`](#persist) is used to mark new entities for future persisting. It will make the entity managed by the [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) and once `flush` will be called, it will be written to the database.

```ts
const user = new User();
user.email = 'foo@bar.com';

// 首先用 `persist()` 标记 Entity，然后 `flush()`
// first mark the entity with `persist()`, then `flush()`
em.persist(user);
await em.flush();

// 我们也可以在这里使用流式 API 并这样做：
// we could as well use fluent API here and do this:
await em.persist(user).flush();
```

要理解 `flush`，让我们首先定义什么是托管 Entity：如果 Entity 是从数据库中获取的（通过 [`em.find()`](#find)），或者通过 [`em.persist()`](#persist) 注册为新 Entity 并在稍后刷新（只有在 `flush` 之后它才成为托管的），那么它就是托管的。

_To understand `flush`, let's first define what managed entity is: An entity is managed if it's fetched from the database (via [`em.find()`](#find)) or registered as new through [`em.persist()`](#persist) and flushed later (only after the `flush` it becomes managed)._

[`em.flush()`](#flush) 将遍历所有托管 Entity，计算适当的变更集并执行相应的数据库查询。由于从数据库加载的 Entity 会自动成为托管的，因此我们不必在这些 Entity 上调用 persist，flush 就足以更新它们。

[`em.flush()`](#flush) will go through all managed entities, compute appropriate change sets and perform according database queries. As an entity loaded from the database becomes managed automatically, we do not have to call persist on those, and flush is enough to update them.

```ts
const user = await em.findOne(User, 1);
user.bio = '...';

// 不需要 persist `user`，因为它已经由 EM 管理
// no need to persist `user` as it's already managed by the EM
await em.flush();
```

让我们尝试在数据库中创建我们的第一条记录，将其添加到 `server.ts` 文件中以代替 `console.log`：

Let's try to create our first record in the database, add this to the `server.ts` file instead of the `console.log`:

```ts title='server.ts'
// 创建新的 user Entity 实例
// create new user entity instance
const user = new User();
user.email = 'foo@bar.com';
user.fullName = 'Foo Bar';
user.password = '123456';

// 首先用 `persist()` 标记 Entity，然后 `flush()`
// first mark the entity with `persist()`, then `flush()`
await orm.em.persist(user).flush();

// Entity 被刷新后，它就成为托管的，并且有了可用的主键
// after the entity is flushed, it becomes managed, and has the PK available
console.log('user id is:', user.id);
```

现在通过 `npm start` 再次运行脚本，你将会看到一个错误：

Now run the script again via `npm start`, and you will see an error:

```text
ValidationError: Using global EntityManager instance methods for context specific actions is disallowed.
If you need to work with the global instance's identity map, use `allowGlobalContext` configuration option
or `fork()` instead.
```

记得我们说过 `orm.em` 是一个全局的 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 实例吗？看起来使用它不是一个好主意，实际上，它默认是禁止的。在我们弄清楚这条消息之前，让我们快速定义两个我们还没有接触过的术语——Identity Map 和 Unit of Work。

Remember we said the `orm.em` is a global [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) instance? Looks like it is not a good idea to use it, in fact, it is disallowed by default. Before we get to the bottom of this message, let's quickly define two more terms we haven't touched yet - the Identity Map and Unit of Work.

- Unit of Work 维护受业务事务影响的对象（Entity）列表，并协调更改的写入。

  _Unit of Work maintains a list of objects (entities) affected by a business transaction and coordinates the writing out of changes._

- Identity Map 通过在映射中保存每个加载的对象，确保每个对象（Entity）只被加载一次。引用对象时使用映射查找对象。

  _Identity Map ensures that each object (entity) gets loaded only once by keeping every loaded object in a map. Looks up objects using the map when referring to them._

MikroORM 是一个数据映射器，试图实现持久化无感知。这意味着你将 JavaScript 对象映射到关系数据库，而该数据库不一定了解数据库。它是如何工作的呢？

MikroORM is a data-mapper that tries to achieve persistence-ignorance. This means you map JavaScript objects into a relational database that doesn't necessarily know about the database at all. How does it work?

### Unit of Work 和 Identity Map

MikroORM 使用 Identity Map 模式跟踪对象。每当你从数据库中获取一个对象时，MikroORM 会在其 `UnitOfWork` 内部保存对此对象的引用。这为 MikroORM 的优化提供了空间。如果你调用 EntityManager 并两次请求具有特定 ID 的 Entity，它将返回相同的实例：

MikroORM uses the Identity Map pattern to track objects. Whenever you fetch an object from the database, MikroORM will keep a reference to this object inside its `UnitOfWork`. This allows MikroORM room for optimizations. If you call the EntityManager and ask for an entity with a specific ID twice, it will return the same instance:

```ts
const jon1 = await em.findOne(Author, 1);
const jon2 = await em.findOne(Author, 1);

// Identity Map 在起作用
// identity map in action
console.log(jon1 === jon2); // true
```

Identity Map 只通过 id 了解对象，因此即使刚刚执行过，具有不同条件的查询也必须访问数据库。但是 MikroORM 首先从行中获取主键，并检查它是否已经在 `UnitOfWork` 内部具有该主键的对象，而不是创建第二个 `Author` 对象。

The Identity Map only knows objects by id, so a query for different criteria has to go to the database, even if it was executed just before. But instead of creating a second `Author` object MikroORM first gets the primary key from the row and checks if it already has an object inside the `UnitOfWork` with that primary key.

### 变更追踪

Identity Map 有第二个更重要的用例。每当你调用 [`em.flush()`](#flush) 时，ORM 将遍历 Identity Map，并对每个 Entity 将原始状态与当前在 Entity 上设置的值进行比较。如果检测到更改，就会将对象排队等待 SQL `UPDATE` 操作。只有更改的字段才会包含在更新查询中。

The identity map has a second, more important use-case. Whenever you call [`em.flush()`](#flush), the ORM will iterate over the Identity Map, and for each entity it compares the original state with the values that are currently set on the entity. If changes are detected, the object is queued for an SQL `UPDATE` operation. Only the fields that changed are part of the update query.

以下代码将使用对 `Author` 对象所做的更改更新你的数据库，即使你没有调用 [`em.persist()`](#persist)：

The following code will update your database with the changes made to the `Author` object, even if you did not call [`em.persist()`](#persist):

```ts
const jon = await em.findOne(Author, 1);

jon.email = 'foo@bar.com';

await em.flush();
```

### 隐式事务

拥有 Unit of Work 的最重要的含义是它允许自动处理事务。

The most important implication of having Unit of Work is that it allows handling transactions automatically.

当你调用 [`em.flush()`](#flush) 时，所有计算出的更改都在数据库事务内查询。这意味着你可以通过调用 [`em.persist()`](#persist) 来控制事务边界，一旦所有更改就绪，调用 `flush()` 将在事务内运行它们。

When you call [`em.flush()`](#flush), all computed changes are queried inside a database transaction. This means that you can control the boundaries of transactions by calling [`em.persist()`](#persist) and once all your changes are ready, calling `flush()` will run them inside a transaction.

> 你也可以通过 `em.transactional(cb)` 手动控制事务边界。
>
> You can also control the transaction boundaries manually via `em.transactional(cb)`.

```ts
const user = await em.findOne(User, 1);

user.email = 'foo@bar.com';
const car = new Car();
user.cars.add(car);

await em.flush();
```

你可以在[事务和并发](../transactions.md)页面中找到有关事务的更多信息。

You can find more information about transactions in [Transactions and concurrency](../transactions.md) page.

### 为什么需要请求上下文？

现在回到关于全局上下文的验证错误。通过刚刚获得的知识，我们知道 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 在 Identity Map 中维护对所有托管 Entity 的引用。想象一下，如果我们在整个应用程序中使用单一的 Identity Map（即单一的全局上下文，全局的 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager)）。它将被所有可以并行运行的请求处理程序共享。

Now back to the validation error about global context. With the freshly gained knowledge, we know [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) maintains a reference to all the managed entities in the Identity Map. Imagine we would use a single Identity Map throughout our application (so a single global context, global [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager)). It will be shared across all request handlers, that can run in parallel.

1. 内存占用不断增长

   由于只有一个共享的 Identity Map，我们不能在请求结束后就清除它。可能有另一个请求正在使用它，因此从一个请求清除 Identity Map 可能会破坏并行运行的其他请求。这将导致内存占用不断增长，因为每个在某个时间点成为托管的 Entity 都会保存在 Identity Map 中。

   As there would be only one shared Identity Map, we can't just clear it after our request ends. There can be another request working with it so clearing the Identity Map from one request could break other requests running in parallel. This will result in a growing memory footprint, as every entity that became managed at some point in time would be kept in the Identity Map.

2. API 端点响应不稳定

   每个 Entity 都有 `toJSON()` 方法，可以自动将其转换为序列化形式。如果我们只有一个共享的 Identity Map，可能会出现以下情况：

   假设有 2 个端点：

    1. `GET /article/:id` 只返回文章，不填充任何内容
    2. `GET /article-with-author/:id` 返回文章并填充其作者

   现在，当有人通过这两个端点请求同一篇文章时，我们可能最终得到两者返回相同的输出：

    1. `GET /article/1` 返回 `Article`，不填充其 `author` 属性
    2. `GET /article-with-author/1` 返回 `Article`，这次填充了 `author`
    3. `GET /article/1` 返回 `Article`，但这次也填充了 `author`

   这是因为关于 Entity 关联被填充的信息存储在 Identity Map 中。

   Every entity has `toJSON()` method, that automatically converts it to serialized form If we have only one shared Identity Map, the following situation may occur:

   Let's say there are 2 endpoints

    1. `GET /article/:id` that returns just the article, without populating anything
    2. `GET /article-with-author/:id` that returns the article and its author populated

   Now when someone requests the same article via both of those endpoints, we could end up with both returning the same output:

    1. `GET /article/1` returns `Article` without populating its property `author` property
    2. `GET /article-with-author/1` returns `Article`, this time with `author` populated
    3. `GET /article/1` returns `Article`, but this time also with `author` populated

   This happens because the information about entity association being populated is stored in the Identity Map.

### Fork 到胜利！

现在我们更好地理解了问题，解决方案是什么？错误建议了它——分叉。通过 `fork()` 方法，我们可以获得一个干净的 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 实例，它具有一个全新的 Unit of Work，带有自己的上下文和 Identity Map。

So we understand the problem better now, what's the solution? The error suggests it - forking. With the `fork()` method we get a clean [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) instance, that has a fresh Unit of Work with its own context and Identity Map.

```ts title='server.ts'
// 首先 fork 以获得独立的上下文
// fork first to have a separate context
const em = orm.em.fork();

// 首先用 `persist()` 标记 Entity，然后 `flush()`
// first mark the entity with `persist()`, then `flush()`
await em.persist(user).flush();
```

再次运行 `npm start`，我们通过了全局上下文验证错误，但只是发现了另一个错误：

Running `npm start` again, we get past the global context validation error, but only to find another one:

```text
TableNotFoundException: insert into `user` (`bio`, `email`, `full_name`, `password`) values ('', 'foo@bar.com', 'Foo Bar', '123456') - no such table: user
```

我们忘记了创建数据库 Schema。幸运的是，我们手头有所需的所有工具。你可以使用 MikroORM 提供的 [`SchemaGenerator`](https://mikro-orm.io/api/knex/class/SqlSchemaGenerator) 来创建 Schema，以及在你更改 Entity 时保持它同步。对于初始测试，让我们使用 `refreshDatabase()` 方法，它对测试很方便——它将首先删除 Schema（如果已经存在），然后根据 Entity 定义（metadata）从头开始创建它。

We forgot to create the database schema. Fortunately, we have all the tools we need at hand. You can use the [`SchemaGenerator`](https://mikro-orm.io/api/knex/class/SqlSchemaGenerator) provided by MikroORM to create the schema, as well as to keep it in sync when you change your entities. For the initial testing, let's use the `refreshDatabase()` method, which is handy for testing - it will first drop the schema if it already exists and create it from scratch based on entity definition (metadata).

```ts title='server.ts'
// 重新创建数据库 Schema
// recreate the database schema
await orm.schema.refreshDatabase();
```

最后，`npm start` 应该成功，如果你在配置中启用了调试模式，你将在日志中看到 SQL 查询，以及最后的 `user.id` 值。

Finally, `npm start` should succeed, and if you enabled the debug mode in your config, you will see the SQL queries in the logs, as well as the `user.id` value at the very end.

```sql
[query] create table `user` (`id` integer not null primary key autoincrement, `full_name` text not null, `email` text not null, `password` text not null, `bio` text not null); [took 1 ms]
[query] pragma foreign_keys = on; [took 0 ms]
[query] begin
[query] insert into `user` (`bio`, `email`, `full_name`, `password`) values ('', 'foo@bar.com', 'Foo Bar', '123456') [took 0 ms]
[query] commit
user id is: 1
```

你可以看到 insert 查询被包裹在一个事务内。这是 Unit of Work 的另一个效果。[`em.flush()`](#flush) 调用将在一个事务内执行所有查询。如果某些操作失败，整个事务将被回滚。

You can see the insert query being wrapped inside a transaction. That is another effect of the Unit of Work. The [`em.flush()`](#flush) call will perform all the queries inside a transaction. If something fails, the whole transaction will be rolled back.

### 获取 Entity

我们在数据库中存储了第一个 Entity。要从那里读取它，我们可以使用 `find()` 和 `findOne()` 方法。

We have our first entity stored in the database. To read it from there we can use `find()` and `findOne()` methods.

```ts title='server.ts'
// 通过主键查找 user，与 `em.findOne(User, { id: 1 })` 相同
// find user by PK, same as `em.findOne(User, { id: 1 })`
const userById = await em.findOne(User, 1);
// 通过邮箱查找 user
// find user by email
const userByEmail = await em.findOne(User, { email: 'foo@bar.com' });
// 查找所有 user
// find all users
const allUsers = await em.find(User, {});
```

我们已经多次提到了 Identity Map——是时候测试它的工作原理了。我们说过 Entity 是托管的，Unit of Work 将追踪其更改，并在我们调用 `flush()` 时计算它们。我们还说过，用 `persist()` 标记的新 Entity 将在刷新后成为托管的。

We mentioned the Identity Map several times already - time to test how it works. We said the entity is managed, and the Unit of Work will track its changes, and compute them when we call `flush()`. We also said a new entity that is marked with `persist()` will become managed after flushing.

将以下代码放入你的 `server.ts` 文件中，就在 `orm.close()` 调用之前：

Put the following code into your `server.ts` file, right before the `orm.close()` call:

```ts title='server.ts'
// user Entity 现在是托管的，如果我们尝试再次查找它，我们会得到相同的引用
// user entity is now managed, if we try to find it again, we get the same reference
const myUser = await em.findOne(User, user.id);
console.log('users are the same?', user === myUser)

// 修改 user 并刷新会产生更新查询
// modifying the user and flushing yields update queries
user.bio = '...';
await em.flush();
```

再次运行 `npm start` 并验证日志：

Run the `npm start` again and verify the logs:

```text
users are the same? true
[query] begin
[query] update `user` set `bio` = '...' where `id` = 1 [took 0 ms]
[query] commit
```

接下来，让我们尝试做同样的事情，但是使用 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) fork：

Next, let's try to do the same, but with an [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) fork:

```ts title='server.ts'
// 现在尝试创建一个新的 fork，从 `orm.em` 还是我们现有的 `em` fork 都无所谓，因为默认我们会得到一个干净的
// now try to create a new fork, does not matter if from `orm.em` or our existing `em` fork, as by default we get a clean one
const em2 = em.fork();
console.log('verify the EM ids are different:', em.id, em2.id);
const myUser2 = await em2.findOneOrFail(User, user.id);
console.log('users are no longer the same, as they came from different EM:', user === myUser2);
```

这将记录以下内容：

Which logs the following:

```text
verify the EM ids are different: 3 4
[query] select `u0`.* from `user` as `u0` where `u0`.`id` = 1 limit 1 [took 0 ms]
users are no longer the same, as they came from different EM: false
```

:::caution 警告 / Caution
我们刚刚使用了 `em.findOneOrFail()` 而不是 `em.findOne()`，正如你可能猜到的，它的目的是始终返回一个值，否则抛出异常。

We just used `em.findOneOrFail()` instead of `em.findOne()`, as you may have guessed, its purpose is to always return a value, or throw otherwise.
:::

你可以看到有一个 select 查询加载 user。这是因为我们使用了一个新的 fork，它默认是干净的——它有一个空的 Identity Map，因此它需要从数据库加载 Entity。在之前的例子中，在我们调用 `em.findOne()` 时它已经存在了。你通过其主键查询 Entity，这样的查询将始终首先检查 Identity Map，并优先从其中获取结果，而不是查询数据库。

You can see there is a select query to load the user. This is because we used a new fork, that is clean by default—it has an empty Identity Map, and therefore it needs to load the entity from the database. In the previous example, we already had it present by the time we were calling `em.findOne()`. You queried the entity by its primary key, and such a query will always first check the identity map and prefer the results from it instead of querying the database.

### 刷新已加载的 Entity

上面描述的行为通常是我们想要的，它作为一级缓存，但如果你总是想重新加载该 Entity，而不管现有状态呢？有几个选项：

The behavior described above is often what we want and serves as a first-level cache, but what if you always want to reload that entity, regardless of the existing state? There are several options:

> [`FindOptions`](https://mikro-orm.io/api/core/interface/FindOptions) 是 `em.find/findOne` 方法的最后一个参数。
>
> [`FindOptions`](https://mikro-orm.io/api/core/interface/FindOptions) is the last parameter of `em.find/findOne` methods.

1. 首先 fork，以获得一个清晰的上下文

   _fork first, to have a clear context_

2. 在 `FindOptions` 中使用 `disableIdentityMap: true`

   _use `disableIdentityMap: true` in the `FindOptions`_

3. 使用 `em.refresh(entity)`

   _use `em.refresh(entity)`_

前两个的效果几乎相同，使用 `disableIdentityMap` 只是在后台为我们做了分叉。让我们谈谈最后一个——刷新。通过 `em.refresh()`，[`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 将忽略 Identity Map 的内容，始终从数据库获取 Entity。

The first two have pretty much the same effect, using `disableIdentityMap` just does the forking for us behind the scenes. Let's talk about the last one - refreshing. With `em.refresh()`, the [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) will ignore the contents of the Identity Map and always fetch the entity from the database.

```ts title='server.ts'
// 更改 user
// change the user
myUser2.bio = 'changed';

// 用 `em.refresh()` 重新加载 user
// reload user with `em.refresh()`
await em2.refresh(myUser2);
console.log('changes are lost', myUser2);

// 让我们再试一次
// let's try again
myUser2!.bio = 'some change, will be saved';
await em2.flush();
```

再次运行 `npm start` 脚本，我们得到以下结果：

Running the `npm start` script again, we get the following:

```text
[query] select `u0`.* from `user` as `u0` where `u0`.`id` = 1 limit 1 [took 1 ms, 1 result]
changes are lost User {
  fullName: 'Foo Bar',
  email: 'foo@bar.com',
  password: '123456',
  bio: '...',
  id: 1
}
[query] begin
[query] update `user` set `bio` = 'some change, will be saved' where `id` = 1 [took 0 ms, 1 row affected]
[query] commit
```

### 删除 Entity

我们涉及了创建、读取和更新 Entity，CRUD 谜题的最后一块是删除操作。要通过 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 删除 Entity，我们有两种可能：

We touched on creating, reading and updating entities, the last piece of the puzzle to the CRUD riddle is the delete operation. To delete entities via [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager), we have two possibilities:

1. 通过 `em.remove()` 标记 Entity 实例——这意味着我们首先需要有 Entity 实例。但不用担心，你甚至可以在不从数据库加载的情况下获得一个——通过 [`em.getReference()`](#getReference)。

   _Mark entity instance via `em.remove()` - this means we first need to have the entity instance. But don't worry, you can get one even without loading it from the database - via [`em.getReference()`](#getReference)._

2. 通过 `em.nativeDelete()` 触发 `DELETE` 查询——当你想要的只是一个简单的删除查询时，就是这么简单。

   _Fire `DELETE` query via `em.nativeDelete()` - when all you want is a simple delete query, it can be simple as that._

让我们测试第一种方法，通过 Entity 实例删除：

Let's test the first approach with removing by entity instance:

```ts title='server.ts'
// 最后，删除 Entity
// finally, remove the entity
await em2.remove(myUser3!).flush();
```

### Entity 引用

那么上面提到的 [`em.getReference()`](#getReference) 方法做什么，什么是 _Entity 引用_ ？

So what does the [`em.getReference()`](#getReference) method mentioned above do and what is an _entity reference_ in the first place?

MikroORM 将每个 Entity 表示为一个对象，甚至那些没有完全加载的也是如此。那些被称为 Entity 引用——它们实际上是常规的 Entity 类实例，但只有其主键可用。这使得可以在不查询数据库的情况下创建它们。引用存储在 Identity Map 中，就像任何其他 Entity 一样。

MikroORM represents every entity as an object, even those that are not fully loaded. Those are called entity references - they are in fact regular entity class instances, but only with their primary key available. This makes it possible to create them without querying the database. References are stored in the identity map just like any other entity.

之前代码片段的一个替代方案也可以是：

An alternative to the previous code snippet could be as well this:

```ts
const userRef = em.getReference(User, 1);
await em.remove(userRef).flush();
```

这个概念对关系特别重要，并且可以与所谓的 `Reference` 包装器结合使用以增加类型安全性，但我们稍后会讲到这一点。

This concept is especially important for relationships and can be combined with the so-called `Reference` wrapper for added type safety, but we will get to that later.

### Entity 状态和 `WrappedEntity`

我们刚才说 Entity 引用是一个常规的 Entity，但只有一个主键。它是如何工作的？在 Entity 发现期间（当你调用 `MikroORM.init()` 时发生），ORM 将修补 Entity 原型并为 `WrappedEntity` 生成一个懒加载 getter——一个保存关于 Entity 的各种 metadata 和状态信息的类。每个 Entity 实例都将有一个，在隐藏的 `__helper` 属性下可用——要以类型安全的方式访问其 API，请使用 `wrap()` helper：

We just said that entity reference is a regular entity, but only with a primary key. How does it work? During entity discovery (which happens when you call `MikroORM.init()`), the ORM will patch the entity prototype and generate a lazy getter for the `WrappedEntity` - a class holding various metadata and state information about the entity. Each entity instance will have one, available under a hidden `__helper` property - to access its API in a type-safe way, use the `wrap()` helper:

```ts
import { wrap } from '@mikro-orm/core';

const userRef = em.getReference(User, 1);
console.log('userRef is initialized:', wrap(userRef).isInitialized());

await wrap(userRef).init();
console.log('userRef is initialized:', wrap(userRef).isInitialized());
```

> 你也可以扩展 MikroORM 提供的 `BaseEntity`。它定义了通过 `wrap()` helper 可用的所有公共方法，所以你可以做 `userRef.isInitialized()` 或 `userRef.init()`。
>
> You can also extend the `BaseEntity` provided by MikroORM. It defines all the public methods available via `wrap()` helper, so you could do `userRef.isInitialized()` or `userRef.init()`.

`WrappedEntity` 实例还保存了 Entity 在加载或刷新时的状态——然后 Unit of Work 在 flush 期间使用这个状态来计算差异。另一个用例是序列化，我们可以使用 `toObject()`、`toPOJO()` 和 `toJSON()` 方法将 Entity 实例转换为普通的 JavaScript 对象。

The `WrappedEntity` instance also holds the state of the entity at the time it was loaded or flushed - this state is then used by the Unit of Work during flush to compute the differences. Another use case is serialization, we can use the `toObject()`, `toPOJO()` and `toJSON()` methods to convert the entity instance to a plain JavaScript object.

## ⛳ 检查点 1 / Checkpoint 1

目前，我们的应用程序由一个单一的 `User` Entity 和一个 `server.ts` 文件组成，在该文件中我们测试了如何使用 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 处理它。你可以在这里找到当前状态的工作 StackBlitz：

Currently, our app consists of a single `User` entity and a `server.ts` file where we tested how to work with it using [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager). You can find working StackBlitz for the current state here:

> 由于 ts-node 中的 ESM 支持的工作方式，在 StackBlitz 项目中不可能使用它——我们需要使用 `node --loader` 代替。我们还使用内存数据库，通过特殊的数据库名称 `:memory:` 可用的 SQLite 功能。
>
> Due to the nature of how the ESM support in ts-node works, it is not possible to use it inside StackBlitz project - we need to use `node --loader` instead. We also use in-memory database, SQLite feature available via special database name `:memory:`.

这是我们目前的 [`server.ts` 文件](https://stackblitz.com/edit/mikro-orm-getting-started-guide-cp-1?file=src%2Fserver.ts)：

This is our [`server.ts` file](https://stackblitz.com/edit/mikro-orm-getting-started-guide-cp-1?file=src%2Fserver.ts) so far:

<iframe width="100%" height="800" frameborder="0" src="https://stackblitz.com/edit/mikro-orm-getting-started-guide-cp-1?embed=1&ctl=1&view=editor&file=src%2Fserver.ts">
</iframe>
