...
到目前为止，我们手动使用 Entity 构造函数创建 Entity 实例。有时我们可能想创建整个 Entity 图，包括关系。你可以使用 [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) 来做到这一点，它是一个同步方法，为你创建 Entity 实例。它允许你创建深层 Entity 图，将关系的外键映射到正确类型的 Entity 引用。此方法还将在创建的 Entity 上调用 [`em.persist()`](#persist)（除非通过 `persistOnCreate` 选项禁用）。

So far we used the entity constructor manually to create an entity instance. Sometimes we might want to create the whole entity graph, including relations. You can use [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) for that, it is a synchronous method that creates the entity instance for you. It allows you to create a deep entity graph, mapping foreign keys of your relations to entity references of the correct type. This method will also call [`em.persist()`](#persist) on the created entity (unless disabled via `persistOnCreate` option).
...
如果你仔细检查了这个代码片段，你可能发现了那个新的神秘的 [`em.clear()`](#clear) 调用。它做什么？它清除 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 的上下文，意味着它将分离它正在管理的所有 Entity。它将使 [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) 恢复到与通过 [`em.fork()`](#fork) 创建新 fork 时相同的状态。你通常在应用中不需要这个，但它对单元测试非常方便，可以模拟新请求的到来。如果你愿意，也可以显式使用 fork。

If you carefully checked this snippet, you probably found that new mysterious [`em.clear()`](#clear) call. What does it do? It clears the context for the [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager), meaning it will detach all the entities it was managing. It will bring the [`EntityManager`](https://mikro-orm.io/api/core/class/EntityManager) to the same state as if you would create a fresh fork via [`em.fork()`](#fork). You won't usually need this in your app, but it is very handy for unit testing, to simulate new requests coming in. You may as well use forks explicitly if you want.
...
它确实有点难看，但如果你仔细看，你会在最开始和最末尾看到重要的细节。这个错误告诉我们，我们传递给 [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) 的对象不完整——它缺少两个属性，`createdAt` 和 `updatedAt` 时间戳。但我们通过属性初始化器为它们定义了默认值，有什么问题吗？

It's indeed a bit ugly, but if you look carefully, you will see the important details at the very beginning and at the very end. This error tells us the object we are passing into [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) is not complete - it is missing two properties, the `createdAt` and `updatedAt` timestamps. But we define the default value for them via property initializer, what's the problem here?
...
通过这个更改，`slug` 和 `description` 属性也是可选的——但 [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) 会抱怨它们。你需要将它们添加到 `OptionalProps` 定义中，就像之前的时间戳一样。但这些是 `Article` Entity 的属性，所以我们应该在 `Article` Entity 中以某种方式做到这一点。也许像这样？

With this change, the `slug` and `description` properties are optional too - but [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) complains about them. You need to add them to the `OptionalProps` definition, as with the timestamps before. But these are the `Article` entity properties, so we should do it in the `Article` entity somehow. Maybe like this?
...
现在 [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) 调用甚至在没有 `slug` 和 `description` 的情况下也能工作：

Now the [`em.create()`](https://mikro-orm.io/api/core/class/EntityManager#create)(#create) call works even without the `slug` and `description`:
